...
public class Day_7 {
    ...
    public int cardResult(List<String> inputs) {
    ...
    public int cardResult_2(List<String> inputs) {
    ...
    static class Hand {
        ...
        public Hand(String cards, Integer value) {
            ...
            List<List<String>> aa = this.cards.stream().filter(s -> !s.equals("J")).collect(Collectors.groupingBy(w -> w)).values().stream().sorted((o1, o2) -> o2.size() - o1.size()).collect(Collectors.toList());
            List<List<String>> bb = aa.stream().filter(a -> a.size() == aa.get(0).size()).collect(Collectors.toList());
            List<List<String>> cc = bb.stream().sorted((o1, o2) -> mapCardScoreWithoutJ.get(o2.get(0)) - mapCardScoreWithoutJ.get(o1.get(0))).collect(Collectors.toList());
        ...
        public int getScore() {
            List<Integer> values = cardsWithoutJ.stream().collect(Collectors.groupingBy(w -> w)).values().stream().map(List::size).collect(Collectors.toList());
            if (values.size() == 1) {
                return 7;
